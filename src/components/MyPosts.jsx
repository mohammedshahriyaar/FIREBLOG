...
function MyPosts({ isAuth }) {
  ...
  const updatePost = (title) => {
    ...
    const postToUpdate = userData.find(
      (postObj) => {
        ...
        return postObj.data().title === title
      }
  ...
  return (
    <div>
      ...
      {userData &&
        userData.map((postObj,idx) => ( 

          // i was using nanoid as key but inside that block i was updating input Field
          // Now, since component keys are changing, react re-renders the UI. Hence loosing focus.
          // What to take from this:
          // Don't use keys which are constantly changing.

          <div key={idx} className="bg-gray-100 p-4 rounded-md mb-4">
            <div className="flex justify-between items-center border-b-2 pb-2 mb-2">
              <p className="text-lg font-bold">
...
//     );
//     setValue("updatedContent", postToUpdate.data().postData);
//   };
...
//   const saveUpdatedPost = async (data) => {
//     await firebase.editPost(editingPost, { postData: data.updatedContent });
//     setEditingPost(null);
//     setValue("updatedContent", "");
//     // Refresh the posts after updating
//     firebase.MyPosts(setUserData);
...
//                     <button
//                       className="text-green-500 mr-2"
//                       onClick={handleSubmit(saveUpdatedPost)}
//                     >
//                       Save
//                     </button>
//                     <button className="text-red-500" onClick={cancelUpdate}>
